...
dense_rank() OVER(order by COUNT(ID) DESC) AS PRO_COM_RANK
FROM MOVIES
...
-- DIRECTOR_NAME-
              -- 1)STEVEN SODERBERGH,SAM LIU ARE THE DIRECTOR WHO HAVE LIST MIN_RATING AND HIGHEST MAX_RATING .
              -- 2) IN ACTION ANTHONY RUSSO , COMEDY MICHAEL POWELL, DRAMA JEMS MANGOLD ARE TOP DIRECTOR WITH RESPECT TO THEIR GENRE.
-- ACTRESSES-
            -- 1)SANGEETA BHAT,FATMIRE SAHITI,PRANATI RAI PRAKASH ARE THE ATCTRESSES WHO HAS MOST NUMBER OF RATING IN DRAMA GENRE  .
			-- 2) PRANATI RAI PRAKASH,LEERA KALJAI,SANAYA IRANI ARE THE MOST MOST RATING ATRESSES IN INDIAN RELEASE MOVIE .
-- PRODUCTION_HOUSE-
             -- 2) LIETOVUS KINOSTUDIJA ,THE ARCHER ARE THE PRODUCTION COMPANY WHO PRODUCES MOST NUMBER OF HIT WITH MORE THEN 8 RATING.
-- ACTOR-
            -- 1)PRIYANKA AUGUSTIN, SHILPA MAHENDER LEADING BY GOPI KRISHNA ARE THE MOSY RATING ACTOR IN INDIA. 
            -- 2) MOHANLAL AND MAMMOOTTY ARE THE ACTOR WHO HAVE MOST MEADIAN RATING OVERALL .
-- GENRE-
			-- 1) COMEDY,DRAMA,THRILLER ARE THE GENRE EHO HAVE THE MOST WORLD_WIDE_INCOME IN LAST THREE YEAR.THESE ARE THE GENRE PEOPLE MOST LIKING.
-- BEST MOTH FOR RELEASE MOVIE-
			-- 5,6,7 AND 12 ARE THOSE MONTHS WHERE LIST AMOUNT OF MOVIE RELEASE SO IT IS A GOOD TIME TO RELEASE A MOVIE BETTER OUTCOME.
